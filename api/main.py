...
from evaluation.ragas_eval import evaluate_response, log_eval, generate_latest
...
if __name__ == '__main__':
    ...
    async def query(request: QueryRequest):
        ...
        if request.include_eval:
            ...
            log_eval(request.question, eval_scores, state["citations"], state["conflicts_detected"])

        return QueryResponse(
            answer=state["final_answer"], 
            citations=state["citations"], 
            confidence_score=state["confidence_score"],
            conflicts_detected=state["confidence_detected"],
